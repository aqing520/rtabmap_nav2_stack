#!/usr/bin/env python3
"""
Navigation bringup — bypasses nav2_bringup's navigation_launch.py to avoid
RewrittenYaml param corruption. Nav2 nodes are launched directly with the
params file passed as-is.

Usage:
  ros2 launch robot_bringup nav2fix.launch.py
  ros2 launch robot_bringup nav2fix.launch.py enable_gps:=true
  ros2 launch robot_bringup nav2fix.launch.py database_path:=/data/maps/my_site/rtabmap.db
"""
...
def generate_launch_description() -> LaunchDescription:
    ...
    # ── 7 & 8. Nav2 + Collision Monitor (via OpaqueFunction) ──
    # We split the shared YAML into per-node files so that each node only
    # receives its own parameters — avoids type clashes from foreign keys.

    def launch_nav2_nodes(context):
        params_path = LaunchConfiguration('nav2_params_file').perform(context)
        auto = LaunchConfiguration('autostart').perform(context).lower() == 'true'
        ...
        # Map: node_name -> (package, executable, extra_remappings)
        nav2_nodes_spec = {
        ...
        nodes = []
        for node_name, (pkg, exe, extra_remap) in nav2_nodes_spec.items():
            # Extract only this node's params from the full yaml
            node_params = full.get(node_name, {}).get('ros__parameters', {})
            # Write to a temp file so ROS2 loads it with correct types
            tmp = tempfile.NamedTemporaryFile(
                mode='w', prefix=f'nav2_{node_name}_', suffix='.yaml', delete=False)
            yaml.dump(
                {node_name: {'ros__parameters': node_params}},
            ...
            nodes.append(Node(
                ...
                output='screen',
                parameters=[tmp.name],
                remappings=nav2_remappings + extra_remap,
            ))
        ...
        nodes.append(Node(
            ...
            parameters=[{
                ...
                'bond_timeout': 10.0,
                'node_names': list(nav2_nodes_spec.keys()),
            }],
        ))
    ...
    nav2_opaque = OpaqueFunction(function=launch_nav2_nodes)
    ...
    # Nav2 nodes (split params per node)
    ld.add_action(nav2_opaque)
